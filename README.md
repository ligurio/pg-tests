...
* full_install — полная установка
  ...
  * проверка исполняемых файлов — проверка подписей в Astra Smolensk; проверка наличия динамически подключаемых библиотеков; проверка наличия отладочных символов >>>
...
* upgrade — обновление до целевой версии
  ...
  * миграция с ванильного PostgreSQL (поставляемого pgdg), например, с PostgreSQL 12 на Postgres Pro STD 12/Postgres Pro ENT 12
  * сверка содержимого базы после обновления с содержимым до (с учётом ожидаемых изменений)
