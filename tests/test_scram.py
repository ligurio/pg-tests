...
host    all             all             ::0/0                   trust"""
        ...
        cursor.execute(
            "CREATE ROLE test_md5_user_auth"
            " WITH PASSWORD 'test_md5_password' LOGIN")
        cursor.execute(
            "CREATE ROLE test_md5_hash_user"
        ...
        cursor = conn.cursor()
        cursor.execute(
            "CREATE ROLE test_scram_user_auth"
            " WITH PASSWORD 'test_scram_password' LOGIN")
        cursor.execute(
            "CREATE ROLE test_scram_hash_user"
            " WITH PASSWORD ('%s' USING \'plain\') LOGIN" % scram_password)
        conn.commit()
        cursor.close()
        conn.close()
        # Step 3
        conn_string_md5_user = \
            "host='localhost' user='test_md5_user_auth'" \
            " password='test_md5_password' dbname='postgres'"
        conn_md5_user = psycopg2.connect(conn_string_md5_user)
        assert conn_md5_user.status == 1
        conn_md5_user.close()
        # Step 4
        conn_string_test_md5_hash_user = \
            "host='localhost' user='test_md5_hash_user'" \
            " password='%s' dbname='postgres'" % md5_password
        ...
        conn_test_scram_user_auth = \
            "host='localhost' user='test_scram_user_auth'" \
            " password='test_scram_password' dbname='postgres'"
        conn_test_scram_user_auth = psycopg2.connect(
            conn_test_scram_user_auth)
        ...
        conn_scram_hash_user = \
            "host='localhost' user='test_scram_hash_user'" \
